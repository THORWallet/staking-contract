...
describe("TGT Staking with 25% treasury fee", function () {
    ...
    describe("should allow deposits and withdraws", function () {
        ...
        it("should allow deposits and withdraws of multiple users", async function () {
            ...
            expect((await tgtStaking.getUserInfo(carol.address, tgt.address))[0]
            ).to.be.equal(utils.parseEther("200"));
            ...
            expect(await tgt.balanceOf(tgtStaking.address)
            ).to.be.closeTo(utils.parseEther("400"), utils.parseEther("0.0001"));

            expect((await tgtStaking.getUserInfo(bob.address, tgt.address))[0]
            ).to.be.closeTo(utils.parseEther("200"), utils.parseEther("0.0001"));
        });
        ...
        it("should allow setting a new deposit fee, only by owner", async function () {
            ...
            await tgtStaking.connect(alice).deposit(utils.parseEther("100"));
            expect(await tgt.balanceOf(alice.address)).to.be.equal(utils.parseEther("900"));
            expect(await tgt.balanceOf(tgtStaking.address)).to.be.equal(utils.parseEther("100"));
            ...
            expect(await tgt.balanceOf(tgtStaking.address)).to.be.equal(
                utils.parseEther("100").add(utils.parseEther("51"))
            );
        ...
        it.only("should claim funds for the treasury consecutively corresponding to the treasury fee", async function () {
            const {
                tgtStaking,
                tgt,
                rewardToken,
            ...
            expect(await rewardToken.balanceOf(alice.address)).to.be.equal(utils.parseEther("25"));
            expect(await rewardToken.balanceOf(bob.address)).to.be.equal(utils.parseEther("25"));
            expect(await rewardToken.balanceOf(carol.address)).to.be.equal(utils.parseEther("25"));
            ...
            expect(await tgtStaking.pendingReward(bob.address, rewardToken.address)).to.be.closeTo(
                utils.parseEther("25"), utils.parseEther("0.0001")
            );
            expect(await tgtStaking.pendingReward(carol.address, rewardToken.address)).to.be.closeTo(
                utils.parseEther("25"), utils.parseEther("0.0001")
            );
            ...
            expect(await rewardToken.balanceOf(alice.address)).to.be.equal(utils.parseEther("50"));
            expect(await rewardToken.balanceOf(bob.address)).to.be.equal(utils.parseEther("50"));
            expect(await rewardToken.balanceOf(carol.address)).to.be.equal(utils.parseEther("50"));
            ...
            await tgtStaking.connect(treasury).treasuryClaim();
            expect(await rewardToken.balanceOf(treasury.address)).to.be.closeTo(utils.parseEther("75"), utils.parseEther("0.0001"));
        });
        ...
        it("should claim accumulated funds for the treasury corresponding to the treasury fee", async function () {
            ...
            await rewardToken.connect(tgtMaker).transfer(tgtStaking.address, utils.parseEther("100"));
            expect(await tgtStaking.pendingTreasuryReward(rewardToken.address)).to.be.closeTo(
                utils.parseEther("75"), utils.parseEther("0.0001")
            );
            await tgtStaking.connect(treasury).treasuryClaim();
            expect(await rewardToken.balanceOf(treasury.address)).to.be.closeTo(utils.parseEther("75"), utils.parseEther("0.0001"));
        });
